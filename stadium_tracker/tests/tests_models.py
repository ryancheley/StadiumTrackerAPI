...
class GameDetailsTest(TestCase):
    def setUp(self) -> None:
        CustomUser.objects.create(username="ryan", favorite_team=119)
    ...
    def test_game_user_combination_is_unique(self):
        ...
        self.assertEquals(unique_together[0][0], "user")
        self.assertEquals(unique_together[0][1], "game_id")

    def test_game_details_properties(self):
        g = GameDetails.objects.get(game_id=1)
        self.assertEqual(g.home_team, "LA Dodgers")

    def test_game_details_ordering(self):
        g = GameDetails.objects.get(game_id=1)
        ordering = g._meta.ordering
        self.assertEquals(ordering[0], "user")
        self.assertEquals(ordering[1], "-game_datetime")
