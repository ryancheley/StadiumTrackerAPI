    ...
    if games_dates is not None:
        for i in range(len(games_dates)):
            ...
            for j in range(len(games_dates[i].get("games"))):
                ...
                away_score = (
                    games_dates[i].get("games")[j].get("teams").get("away").get("score")
                )
                home_score = (
                    games_dates[i].get("games")[j].get("teams").get("home").get("score")
                )
                ...
                data = {"text": text, "gamePk": gamePk, "venue_id": venue_id}
    ...
    if r.json().get("totalItems") > 0:
        for g in range(min(r.json().get("totalItems"), 3)):
            game_time = datetime.strptime(
                r.json().get("dates")[0].get("games")[g].get("gameDate"),
                "%Y-%m-%dT%H:%M:%SZ",
            ).time()
            game.append(
                {
